...
class ForumController extends GetxController {
  ...
  Future<void> fetchForum() async {
    debugPrint(
        'ForumController: fetchForum called, page=${page.value}, isLoading=${isLoading.value}');
    if (isLoading.value) {
    ...
    debugPrint(
        'ForumController: Token ${token != null ? "received" : "not available"}');

    try {
      debugPrint(
          'ForumController: Calling API to get forums for page ${page.value}');
      final response = await forumProvider.getForum(page.value, token);
      debugPrint(
          'ForumController: API response status: ${response.statusCode}, text: ${response.statusText}');

      if (response.statusCode == 200) {
        try {
          final responseBody = response.body;
          debugPrint(
              'ForumController: Response body type: ${responseBody.runtimeType}');
          ...
          if (data is List && data.isNotEmpty) {
            debugPrint(
                'ForumController: Data is a non-empty list of length ${data.length}');
            try {
              ...
              debugPrint(
                  'ForumController: Processed ${newForums.length} new forums');
            ...
            debugPrint(
                'ForumController: No forum data available or empty list');
            if (page.value == 1) {
        ...
        debugPrint(
            'ForumController: Error response: ${response.statusCode}, ${response.statusText}');
        debugPrint('ForumController: Error body: ${response.body}');
        Get.snackbar(
            'Error', 'Gagal mengambil data forum: ${response.statusText}');
        hasMore.value = false;
      }
    } catch (e, stackTrace) {
      ...
      debugPrint(
          'ForumController: Completed fetchForum, setting isLoading to false');
  ...
  void loadMore() {
    debugPrint(
        'ForumController: loadMore called, isLoading=${isLoading.value}, hasMore=${hasMore.value}');
    if (!isLoading.value && hasMore.value) {
      fetchForum();
    } else {
      debugPrint(
          'ForumController: Skipping loadMore because ${isLoading.value ? "still loading" : "no more data"}');
    }
  }
  ...
  Future<void> fetchKomentar(int id) async {
    ...
    try {
      ...
      debugPrint(
          'ForumController: Komentar API response status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        try {
          ...
          if (data is List) {
            ...
            debugPrint(
                'ForumController: Loaded ${komentarForum.length} comments');
          } else {
            debugPrint(
                'ForumController: No comments available or invalid format');
            komentarForum.value = [];
          }
        } catch (e, stackTrace) {
  ...
  Future<void> fetchForumDetail(int id) async {
    ...
    try {
      final String? token = await TokenStorage.getToken();
      debugPrint(
          'ForumController: Token for detail ${token != null ? "received" : "not available"}');

      final response = await forumProvider.getForumDetail(id, token);
      debugPrint(
          'ForumController: Detail API response status: ${response.statusCode}');
 ...
 Future<void> forumByUser() async {
  ...
  Future<void> hapusForum(int? id) async {
    final String? token = await TokenStorage.getToken();
    final response = await forumProvider.deleteForumUser(token, id);
    if (response.statusCode == 200) {
      Get.snackbar('Sukses', 'Data Berhasil dihapus');
    } else {
      Get.snackbar('Gagal', response.body['message']);
    }
  }
  ...
  Future<void> buatKomentar(String komentar, int forum_id) async {
    debugPrint(
        'ForumController: buatKomentar called for forum ID $forum_id with comment: $komentar');
    try {
      final String? token = await TokenStorage.getToken();
      if (token == null) {
      ...
      debugPrint(
          'ForumController: Comment API response status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            'ForumController: Error posting comment: ${response.statusCode}, ${response.statusText}');
        debugPrint('ForumController: Error body: ${response.body}');
